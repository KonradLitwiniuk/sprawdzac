...
def build_group_url(unit_id: str, group_number: str) -> str:
    # UWAGA: safe="/" jest tu kluczowe. Bez tego urlencode zamienia ukosniki
    # w "_action" na %2F, a USOSweb odpowiada wtedy 404 - routing dziala na
    # surowym tekscie, nie na zdekodowanym.
...
# USOSweb trzyma sesje w ciasteczku PHPSESSID i przy pierwszym wejsciu
# przekierowuje, zeby je zalozyc. Bez pamietania ciasteczek konczy sie to
# petla przekierowan ("HTTP Error 302 ... infinite loop"), wiec kazde
# zapytanie idzie przez wspolny opener z sloikiem na ciasteczka.
...
def prime_session(timeout: int = 20) -> None:
    """Odwiedz strone glowna USOSweba, zeby dostac ciasteczko sesji."""
    ...
    try:
        _OPENER.open(
            _build_request(f"{USOS_WEB_URL}/kontroler.php?_action=news/default"),
            timeout=timeout,
        ).read(4096)
        names = sorted(cookie.name for cookie in _COOKIES)
        log(f"  sesja USOSweb: ciasteczka {names or 'BRAK'}")
    except Exception as error:  # noqa: BLE001 - brak sesji nie musi byc koncem
        log(f"  (nie udalo sie zalozyc sesji USOSweb: {error})")
...
def fetch(url: str, timeout: int = 30, retries: int = 3) -> str:
    ...
    # Sesja USOSweba wygasa po kilkudziesieciu minutach. Przy dlugim chodzeniu
    # w petli objawia sie to znowu petla przekierowan, wiec po serii bledow
    # kasujemy sesje - nastepne pobranie zalozy ja od nowa.
    global _SESSION_PRIMED
    _SESSION_PRIMED = False
    _COOKIES.clear()
...
def ntfy_notify(topic: str, title: str, message: str, click: str = "") -> None:
    ...
    go w --ntfy. Temat jest jak haslo: kto zna nazwe, ten czyta powiadomienia,
    wiec nie uzywaj nazwy z zadnego przykladu ani niczego oczywistego."""
    try:
...
def discover_units(course_code: str, debug: bool = False) -> list[str]:
    ...
    for url in course_page_urls(course_code):
        try:
            ...
            log(f"  proba: {url}\n      -> nie udalo sie pobrac ({error})")
            continue

        ids = re.findall(r"zaj_cyk_id=(\d+)", html.unescape(page))
        log(
            f"  proba: {url}\n"
            f"      -> strona ma {len(page)} znakow, zaj_cyk_id: "
            f"{sorted(set(ids)) or 'BRAK'}"
        )
        if debug:
            snippet = strip_tags(main_content(page))[:400]
            log(f"      DEBUG poczatek tresci: {snippet!r}")
        for unit in ids:
...
def main() -> int:
    ...
    if args.once:
        # Zwracamy blad, gdy nic nie znaleziono - dzieki temu uruchomienie
        # w GitHub Actions swieci na czerwono zamiast udawac, ze wszystko gra.
        if check_once(args, notifier):
            return 0
        log("NIC NIE ZNALEZIONO - patrz logi wyzej.")
        return 1
